(() => {
  ...
  function metricCards() {
    return [
      ...
      { key: "grossReturn", title: "Rendimento totale lordo", value: pct(totals.netContributed ? totals.grossProfit / totals.netContributed * 100 : 0), hint: "su capitale conferito", essential: true },
      { key: "netReturn", title: "Rendimento totale netto", value: pct(totals.netContributed ? totals.netProfit / totals.netContributed * 100 : 0), hint: "su capitale conferito", essential: true },
      ...
      { key: "openCost", title: "Capitale ancora investito", value: eur(totals.openCost), hint: "cost basis aperto", essential: true },
  ...
  function renderAllocation() {
    ...
    $("allocation").innerHTML = all.length ? all.map(row => {
      ...
      return `<div class="allocation-row"><div><strong>${safe(row.name)}</strong><div class="small">${safe(row.sub)} · ${eur(row.value)} · ${pct(p)}</div></div><div class="barbox"><div class="bar" style="width:${Math.max(2, Math.min(100, p))}%"></div></div></div>`;
  ...
  function renderTradeLog() {
    ...
    $("tradeLog").innerHTML = rows.length ? rows.map(row => `<div class="log-item"><strong>${safe(row.date)} · ${row.side === "sell" ? "Vendita" : "Acquisto"} · ${safe(assetName(row.asset_id))}</strong><div class="small">${fmt4(row.quantity)} × ${eur(row.price)} · controvalore ${eur(row.amount)}${row.side === "sell" ? "" : ""}</div>${row.note ? `<p class="small">${safe(row.note)}</p>` : ""}</div>`).join("") : `<p class="small">Nessun movimento.</p>`;
  }
  ...
  function renderEntryLog() {
    ...
    holder.innerHTML = rows.length ? rows.map(row => `<div class="log-item"><strong>${safe(row.date)} · ${safe(row.characteristic || blockName(row.generic_option))} ${row.transaction_type === "buy" ? "Acquisto" : "Aggiornamento"}</strong><div class="small">investito ${eur(row.purchase_price)} · valore ${eur(row.current_value)}${n(row.current_price) ? ` · prezzo ${eur(row.current_price)}` : ""}${n(row.cash_value) ? ` · cash ${eur(row.cash_value)}` : ""}</div>${row.text_value ? `<p class="small">${safe(row.text_value)}</p>` : ""}</div>`).join("") : `<p class="small">Nessun movimento snapshot.</p>`;
  }
  ...
  async function saveTrade() {
    ...
    const row = { user_id: userId, id: uid("investment-trade"), asset_id: assetId, date: $("tradeDate").value || today(), side: tradeSide, quantity, price, amount, note: $("tradeNote").value || "", updated_at: new Date().toISOString() };
  ...
  async function savePrice(assetId) {
    ...
    await client.from("investment_entries").insert({ user_id: userId, id: uid("price-snapshot"), date, characteristic: asset.name, number_value: pos.openQty, purchase_price: pos.openCost, current_price: price, current_value: pos.marketValue, cash_value: 0, text_value: "Aggiornamento prezzo da app Investimenti separata", generic_option: asset.block_id, transaction_type: "update", ath: 0, created_at_ms: Date.now(), updated_at: new Date().toISOString() });
    await loadCloud();
